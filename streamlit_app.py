...
if prompt := st.chat_input("Ask about US Census data..."):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            error_msg = str(e)
            if error_code == 250001 or "auth" in error_msg.lower() or "JWT" in error_msg or "token" in error_msg.lower():
                # Auth/token failure — clear cache and retry once with a fresh connection
                get_snowflake_connection.clear()
                try:
                    _show_status("Refreshing connection...")
                    conn = _get_healthy_connection()
                    response, updated_history = process_query(
                        conn,
                        st.session_state.conversation_history,
                    ...
                    response = (
                        ...
                        "This usually means:\n"
                        "- The private key in secrets.toml doesn't match the public key registered on the Snowflake user\n"
                        "- The account identifier is wrong (should be `orgname-accountname`)\n\n"
                        f"Detail: {str(retry_e)[:200]}"
                    )
            ...
            # Connection dropped — clear cache so next request reconnects
            get_snowflake_connection.clear()
            response = (
            ...
            elif "JWT" in error_msg or "token" in error_msg.lower() or "401" in error_msg:
                # Catch JWT/token errors that surface as generic exceptions
                get_snowflake_connection.clear()
                response = (
                    "**Token error.** The authentication token was rejected.\n\n"
                    "I've cleared the cached connection — please try your question again."
                )
            else:
                response = (
