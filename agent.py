...
from guardrails import (
    validate_input, validate_output, ValidationResult, sanitize_input
)
...
def validate_sql(sql):
    ...
    allowed_tables = [
        ...
        'CENSUS_AGENT.PUBLIC.V_MARITAL_STATUS',
        'CENSUS_AGENT.PUBLIC.V_SCHOOL_ENROLLMENT',
        'CENSUS_AGENT.PUBLIC.V_EARNINGS',
        'CENSUS_AGENT.PUBLIC.V_OCCUPATION',
        'CENSUS_AGENT.PUBLIC.V_MOBILITY',
        ...
        'CENSUS_AGENT.PUBLIC.V_STATE_MARITAL_STATUS',
        'CENSUS_AGENT.PUBLIC.V_STATE_SCHOOL_ENROLLMENT',
        'CENSUS_AGENT.PUBLIC.V_STATE_EARNINGS',
        'CENSUS_AGENT.PUBLIC.V_STATE_OCCUPATION',
        'CENSUS_AGENT.PUBLIC.V_STATE_MOBILITY'
    ]
...
def _execute_with_retry(conn, sql, explanation, messages, deadline=None):
    """Execute SQL with agentic retry — feed errors back to the LLM for correction.
    Respects the wall-clock deadline; aborts if time runs out."""
    import time
    import pandas as pd

    if deadline is None:
        deadline = time.time() + 50  # default budget if not passed
    ...
    for attempt in range(MAX_RETRIES):
        if time.time() > deadline:
            return (
                "**Request timed out.** The query took too long (60s limit). "
        ...
        try:
            ...
            if time.time() > deadline - 10:
                break  # not enough time for another LLM call
...
def process_query(conn, conversation_history, user_message):
    ...
    Validation pipeline:
      1. Input sanitization (injection, length)
      2. Topic classification (keyword fast-path)
      3. LLM generates SQL
      4. Output validation (SQL safety + grounding)
      5. Execute with retry

    Hard wall-clock limit: 60s total from entry to return.
    """
    import time
    import pandas as pd
    ...
    # --- Stage 1+2: Input validation pipeline ---
    guard_result = validate_input(user_message, conversation_history)
    if guard_result.result == ValidationResult.BLOCK:
        return guard_result.message, conversation_history
    ...
    if time.time() > deadline:
        return (
            "**Request timed out.** The AI service took too long to respond (60s limit). "
    ...
    if action == "refuse":
        ...
        # --- Stage 3: Output validation (safety + grounding) ---
        output_check = validate_output(sql)
        if output_check.result == ValidationResult.BLOCK:
            response = (
                f"I generated a query but it failed validation: {output_check.message}\n\n"
                "Please rephrase your question."
            )
        else:
            response = _execute_with_retry(conn, sql, explanation, messages, deadline)
    else:
